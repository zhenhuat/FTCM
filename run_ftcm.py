...
from model.my_ftcm_drop import Model
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == 'train':
            ...
            if opt.MAE:
                ...
                # for seq_cnt in range(len(subject)):
                #     seq_name = subject[seq_cnt]
                #     #print(seq_name)
                #     #exit()
                #     if seq_name in data_inference:
                #         data_inference[seq_name] = np.concatenate((data_inference[seq_name], pred_out[seq_cnt].permute(2, 1, 0).cpu().numpy()), axis=2)
                #         #data_2d[seq_name] = np.concatenate((data_2d[seq_name], out[seq_cnt].permute(2, 1, 0).cpu().numpy()), axis=2)
                #         data_3d_gt[seq_name] = np.concatenate((data_3d_gt[seq_name], out_target[seq_cnt].permute(2, 1, 0).cpu().numpy()), axis=2)
                #     else:
                #         data_inference[seq_name] = pred_out[seq_cnt].permute(2, 1, 0).cpu().numpy()
                #         data_3d_gt[seq_name] = out_target[seq_cnt].permute(2, 1, 0).cpu().numpy()
    ...
    if split == 'train':
        ...
        if opt.refine:
            ...
            #     mat_path = os.path.join(opt.checkpoint, 'inference_data_h36_all_ftcm27.mat')
            #     scio.savemat(mat_path, data_inference)

            #     data_3d_gt[seq_name] = data_3d_gt[seq_name][:, :, None, :]
            #     mat_path_gt = os.path.join(opt.checkpoint, 'inference_data_h36_all_gt_ftcm27.mat')
            #     scio.savemat(mat_path_gt, data_3d_gt)
